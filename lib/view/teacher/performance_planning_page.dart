...
class _SubKPIListPageState extends State<SubKPIListPage> {
  ...
  Future<void> _fetchKPIData() async {
    try {
      ...
      var detailReq = await http.post(
        Uri.parse(
          'https://schoolapp-api-dev.zeabur.app/api/teacherperf/plan/detail',
        ),
        headers: {
      ...
      if (planHId.isNotEmpty) {
        var listKpiReq = await http.post(
          Uri.parse(
            'https://schoolapp-api-dev.zeabur.app/api/teacherperf/plan/detail/list-kpi',
          ),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
  ...
  void _showKPIDialog({Map<String, dynamic>? existingKpi, int? index}) {
    ...
    final codeCtrl = TextEditingController(
      text: existingKpi?['code'] ?? "KPI_NEW",
    );
    final nameCtrl = TextEditingController(text: existingKpi?['name'] ?? "");
    final descCtrl = TextEditingController(
      text: existingKpi?['description'] ?? "-",
    );
    final weightCtrl = TextEditingController(
      text: existingKpi?['weight'] ?? "",
    );
    final targetCtrl = TextEditingController(
      text: existingKpi?['target'] ?? "",
    );
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text(
              isEdit ? "Edit Performance Plan KPI" : "Add Performance Plan KPI",
  ...
  Widget _buildDialogField(
    ...
    return TextField(
      ...
      style: TextStyle(
        ...
        color: isReadOnly ? Colors.blueGrey.shade700 : Colors.black87,
      ),
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          ...
          borderSide: BorderSide(
            color: isReadOnly ? Colors.grey.shade300 : Colors.indigo.shade400,
            width: 1.5,
          ),
        ),
      ),
        ...
        actions: [
          if (!isApproved)
            Container(
              ...
              child: IconButton(
                onPressed: () => _showKPIDialog(),
                icon: Icon(Icons.add_task_rounded, color: Colors.blue.shade600),
              ),
            ),
        ],
      ),

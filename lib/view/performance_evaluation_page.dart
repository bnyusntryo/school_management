...
class _SubEvaluationFormPageState extends State<SubEvaluationFormPage> {
  ...
  Future<void> _fetchEvaluationData() async {
    try {
      ...
      var req = await http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/eval/detail'),
        ...
        if (res['data'] != null) {
          ...
          if (mounted) {
            setState(() {
              ...
              _attendanceScoreCtrl.text = data['score_attendance']?.toString() ?? "0";
              _developmentScoreCtrl.text = data['score_teacherdev']?.toString() ?? "0";
              _attitudeScoreCtrl.text = data['score_etitut']?.toString() ?? "0";

              _finalScore = double.tryParse(data['total_score']?.toString() ?? "0") ?? 0.0;
              _isCalculated = true;
              _isLoading = false;
            });
          }
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Icon(Icons.assignment_ind_rounded, color: Colors.green.shade600, size: 20),
                            const SizedBox(width: 8),
                            const Text("Evaluation Info", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          ],
                        ),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Icon(Icons.assessment_rounded, color: Colors.green.shade600),
                            const SizedBox(width: 8),
                        ...
                        _buildEvalInput(label: "Score Attendance", controller: _attendanceScoreCtrl, isReadOnly: true),
                        _buildEvalInput(label: "Score Teacher Development", controller: _developmentScoreCtrl, isReadOnly: true, trailingWidget: _buildDropdown1to4(_developmentScoreCtrl)),
                        _buildEvalInput(label: "Score Teacher Attitude", controller: _attitudeScoreCtrl, isReadOnly: true, isRequired: true, trailingWidget: _buildDropdown1to4(_attitudeScoreCtrl)),
                        ...
                        if (_isCalculated)
                          Container(
                            ...
                            padding: const EdgeInsets.all(20),
                            decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(15), border: Border.all(color: Colors.green.shade200, width: 2)),
                            child: Column(
                              children: [
                                ...
                                TweenAnimationBuilder<double>(
                                  ...
                                  builder: (context, value, child) {
                                    return Text(value.toStringAsFixed(6), style: TextStyle(fontSize: 32, color: Colors.green.shade700, fontWeight: FontWeight.w900));
                                  },
                                ),
                              ],
                            ),
      ...
      floatingActionButton: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(flex: 2, child: ElevatedButton.icon(onPressed: () { _calculateFinalScore(); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Simulasi Kalkulasi Berhasil (API Save Dinonaktifkan)"), backgroundColor: Colors.green)); }, icon: const Icon(Icons.calculate_rounded, color: Colors.white), label: const Text("Calculate", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade600, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), elevation: 4, shadowColor: Colors.green.withOpacity(0.4)))),
          ],
        ),
      ),
    );
  ...
  Widget _buildInfoRow(String label, String value) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(width: 100, child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey.shade500))),
        const Text(" :  ", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
        Expanded(child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.black87))),
      ],
    );
  }

  Widget _buildEvalInput({required String label, required TextEditingController controller, bool isReadOnly = false, bool isRequired = false, Widget? trailingWidget}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 15),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          RichText(text: TextSpan(text: label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey.shade700, fontFamily: 'Roboto'), children: [if (isRequired) const TextSpan(text: " *", style: TextStyle(color: Colors.red))])),
          const SizedBox(height: 8),
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    filled: true,
                    fillColor: isReadOnly ? const Color(0xFFE8ECEF) : Colors.white,
              ...
              if (trailingWidget != null) ...[const SizedBox(width: 10), trailingWidget]
            ],
          ),
        ],
      ),
...
class SubEvaluationKpiListModal extends StatefulWidget {
  ...
  const SubEvaluationKpiListModal({super.key, required this.teacherData, this.periodData});

  @override
  State<SubEvaluationKpiListModal> createState() => _SubEvaluationKpiListModalState();
}
...
class _SubEvaluationKpiListModalState extends State<SubEvaluationKpiListModal> {
  ...
  Future<void> _fetchDetailedKpis() async {
    try {
      ...
      var planReq = http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/eval/detail/plan'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'},
        body: jsonEncode({
      ...
      var listKpiReq = http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/eval/detail/list-kpi'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'},
        body: jsonEncode({
          "perfperiod_id": periodId,
          "perfplanh_id": planHId,
        ...
        setState(() {
          if (planRes.statusCode == 200) {
            ...
            if (decodedPlan['data'] != null) {
              _planStatus = decodedPlan['data']['status']?.toString() ?? "Unknown";
            }
          }
  ...
  Future<void> _fetchAndShowKpiDetail(String perfplandId) async {
    ...
    try {
      ...
      var req = await http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/plan/detail/detail-kpi'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'},
        body: jsonEncode({
          "perfplanh_id": planHId,
      ...
      if (req.statusCode == 200) {
        ...
        if (res['data'] != null) {
          ...
          if (data is List && data.isNotEmpty) {
            targetData = data.firstWhere((element) => element['perfpland_id'] == perfplandId, orElse: () => data[0]);
          } else {
            targetData = data;
          }
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Gagal mengambil detail KPI"), backgroundColor: Colors.red));
      }
    } catch (e) {
      Navigator.pop(context);
  ...
  Future<void> _fetchAndShowMonitoringDetail(String perfplandId) async {
    ...
    try {
      ...
      var req = await http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/monit/detail/detail-kpi'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'},
        body: jsonEncode({
          "perfplanh_id": planHId,
          "perfpland_id": perfplandId
      ...
      if (req.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Gagal mengambil data Monitoring"), backgroundColor: Colors.red));
      }
    } catch (e) {
      Navigator.pop(context);
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          ...
          children: [
            ...
            InkWell(onTap: () => Navigator.pop(context), child: const Icon(Icons.close_rounded, color: Colors.grey, size: 20)),
          ],
        ),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildDialogReadOnlyField("KPI Code", kpiData['kpi_code']?.toString() ?? '-'),
              const SizedBox(height: 12),
              _buildDialogReadOnlyField("KPI Name", kpiData['kpilibrary_name']?.toString() ?? '-'),
              const SizedBox(height: 12),
              _buildDialogReadOnlyField("KPI Description", kpiData['kpilibrary_desc']?.toString() ?? '-'),
              const SizedBox(height: 12),
              _buildDialogReadOnlyField("Weight *", kpiData['weight']?.toString() ?? '-'),
              const SizedBox(height: 12),
              _buildDialogReadOnlyField("Target *", kpiData['target']?.toString() ?? '-'),
            ],
          ),
        ),
      ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text("Monitoring Detail", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1A237E))),
            InkWell(onTap: () => Navigator.pop(context), child: const Icon(Icons.close_rounded, color: Colors.grey, size: 20)),
          ],
        ),
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("KPI Information", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.black87)),
                const SizedBox(height: 10),
                _buildDialogReadOnlyField("KPI Code", kpiData['kpi_code']?.toString() ?? '-'),
                const SizedBox(height: 10),
                _buildDialogReadOnlyField("KPI Name", kpiData['kpilibrary_name']?.toString() ?? '-'),
                ...
                const Text("Monthly Monitoring", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.black87)),
                const SizedBox(height: 10),
                ...
                GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    String val = kpiData['perfmonit_${index + 1}']?.toString() ?? "0";
                    if (val == "0.0") val = "0";
                    return Column(
                      children: [
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: Text(val, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black87)),
                          ),
                        ),
                      ],
                    );
      ...
      children: [
        RichText(
          text: TextSpan(
              ...
              children: [if (label.contains("*")) const TextSpan(text: " *", style: TextStyle(color: Colors.red))]
          ),
        ),
        const SizedBox(height: 8),
        TextField(
  ...
  Widget _buildInfoRow(String label, String value, {bool isStatus = false}) {
    ...
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(width: 130, child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey.shade500))),
        const Text(" :  ", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
        Expanded(
          child: isStatus
              ? Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: isApproved ? Colors.green.shade50 : Colors.red.shade50, borderRadius: BorderRadius.circular(6), border: Border.all(color: isApproved ? Colors.green.shade200 : Colors.red.shade200)),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text("Evaluation KPI List", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xFF1A237E))),
          ...
          if (_isLoading)
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  _buildInfoRow("Teacher User ID", widget.teacherData['id'] ?? '-'),
                  const SizedBox(height: 8),
                  _buildInfoRow("Teacher Name", widget.teacherData['name'] ?? '-'),
                  const SizedBox(height: 8),
                  _buildInfoRow("Evaluation Status", _planStatus, isStatus: true),
                ],
              ),
            ),
            ...
            Expanded(
              child: _kpiList.isEmpty
                  ? Center(child: Text("No KPI data found.", style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.bold)))
                  : ListView.builder(
                        ...
                        children: [
                          Row(
                            children: [
                              Container(
                                ...
                                child: Text(kpi['kpi_code'], style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.indigo.shade700)),
                              ),
                            ],
                          ),
                          const SizedBox(height: 10),
                          ...
                          Row(
                            children: [
                              ...
                              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("WEIGHT", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey.shade500)), const SizedBox(height: 4), Text(kpi['weight'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14))])),

                              Row(
                                children: [
                                  InkWell(
                                    onTap: () => _fetchAndShowKpiDetail(kpi['perfpland_id']),
                                    child: Container(
                                  ...
                                  InkWell(
                                    onTap: () => _fetchAndShowMonitoringDetail(kpi['perfpland_id']),
                                    child: Container(
                                      padding: const EdgeInsets.all(8),
                                      decoration: BoxDecoration(color: Colors.blue.shade600, shape: BoxShape.circle),
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text("SCORE", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.indigo.shade500)),
                                      const SizedBox(height: 4),
                                      Container(
                                        height: 35,
                                        ...
                                        child: DropdownButtonHideUnderline(
                                          child: DropdownButton<String>(
                                            value: ["1", "2", "3", "4"].contains(kpi['score']) ? kpi['score'] : "1",
                                            icon: Icon(Icons.keyboard_arrow_down_rounded, color: Colors.blue.shade700, size: 16),
                                            items: ["1", "2", "3", "4"].map((String val) {
                                              return DropdownMenuItem<String>(value: val, child: Text(val, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)));
                                            }).toList(),
                                            onChanged: (String? newValue) {
                                              if (newValue != null) {
                                                setState(() { kpi['score'] = newValue; });
                                              }
                                            },
                                          ),
                                        ),

  ...
  Future<void> _fetchMonitoringData() async {
    try {
      ...
      var detailReq = http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/monit/detail'),
      ...
      var listKpiReq = http.post(
        Uri.parse('https://schoolapp-api-dev.zeabur.app/api/teacherperf/monit/detail/list-kpi'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'},
        body: jsonEncode({
          "perfperiod_id": periodId,
        ...
        setState(() {
          if (detailRes.statusCode == 200) {
            ...
            if (decodedDetail['data'] != null) {
              _monitStatus = decodedDetail['data']['status']?.toString() ?? "Unapproved";
            }
          }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.monitor_heart_rounded, color: Colors.orange.shade600, size: 20),
              const SizedBox(width: 8),
              const Text("Monitoring Information", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Monitoring Progress", style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 18)),
      ),
      body: _isLoading
        ...
        children: [
                ...
                children: [
                  ElevatedButton.icon(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Monitoring Form Approved!"), backgroundColor: Colors.green));
                    },
                    icon: const Icon(Icons.check_rounded, color: Colors.white, size: 16),
                    label: const Text("Approve Monitoring Form", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
          ...
          Expanded(
            child: kpis.isEmpty
                ? Center(child: Text("No KPI available.", style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.bold)))
                : ListView.builder(
              ...
              itemBuilder: (context, index) => Container(
                ...
                child: Material(
                  ...
                  child: ListTile(
                    ...
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => SubMonthlyInputPage(
                        kpi: kpis[index],
                        planHId: widget.teacherData['perfplanh_id'] ?? ''
                    ))),

                    leading: Container(
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(10)),
                        child: Icon(Icons.insert_chart_rounded, color: Colors.orange.shade600)
                    ),
...
class SubMonthlyInputPage extends StatefulWidget {
  ...
  const SubMonthlyInputPage({super.key, required this.kpi, required this.planHId});
...
class _SubMonthlyInputPageState extends State<SubMonthlyInputPage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < 12; i++) {
      _controllers.add(TextEditingController(
          text: widget.kpi['monthlyData'][i] == "0" ? "" : widget.kpi['monthlyData'][i]
      ));
    }
    _fetchMonthlyData();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5F7FA),
      appBar: AppBar(backgroundColor: Colors.white, elevation: 0, iconTheme: const IconThemeData(color: Colors.black87), title: Text(widget.kpi['name'], style: const TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 16))),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator(color: Colors.orange))
          : Column(
        children: [
          Container(
            ...
            child: Text("Target Score: ${widget.kpi['target']}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18), textAlign: TextAlign.center),
          ),
          Expanded(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          onChanged: (v) => widget.kpi['monthlyData'][index] = v.isEmpty ? "0" : v,
                        ),
                      ),
                    ],
                  ),
      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(
          child: ElevatedButton.icon(
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Fitur Save API menunggu intelijen berikutnya!"), backgroundColor: Colors.orange));
            },
            icon: const Icon(Icons.save_rounded, color: Colors.white),
            label: const Text("Save Progress", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            style: ElevatedButton.styleFrom(
